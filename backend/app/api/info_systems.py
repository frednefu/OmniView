...
def list_supply_chain(page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100),
    ...
    if search:
        # 多条件搜索：单位名称 / 数据添加者 / 认领人
        from sqlalchemy import or_
        user_ids = [u[0] for u in db.query(User.id).filter(User.name.contains(search)).all()]
        conditions = [SupplyChain.company_name.contains(search)]
        if user_ids:
            conditions.append(SupplyChain.created_by.in_(user_ids))
            conditions.append(SupplyChain.claimed_by.in_(user_ids))
