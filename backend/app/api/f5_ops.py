...
def list_virtual_servers(
    ...
    items = [F5OpsVSItem(**item) for item in data]
    total = len(items)
    return PaginatedF5OpsResponse(items=items, total=total, page=1, size=total, pages=1)


# ══════════════════════════════════════════════════════════════════
# 端点 2：资源池（返回全量数据，前端分页筛选）
# ══════════════════════════════════════════════════════════════════
...
def list_pools(
    ...
    items = [F5OpsPoolItem(**item) for item in data]
    total = len(items)
    return PaginatedF5OpsResponse(items=items, total=total, page=1, size=total, pages=1)


# ══════════════════════════════════════════════════════════════════
# 端点 3：iRules（返回全量数据，前端分页筛选）
# ══════════════════════════════════════════════════════════════════
...
def list_rules(
    ...
    items = [F5OpsRuleItem(**item) for item in data]
    total = len(items)
    return PaginatedF5OpsResponse(items=items, total=total, page=1, size=total, pages=1)
