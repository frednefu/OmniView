...
- **仪表盘** — 全源资源概览：统计卡片（ZDNS域名/F5域名/公网IP端口/内网服务/虚拟机/VM IP/VLAN/文件夹）点击弹出明细搜索对话框，VM 开关机/Pool 状态/DNS 记录类型/扫描次数环形图与饼图，地址段利用率图表点击下探已占用 IP，ESXi CPU 类型分布/存储类型容量统计/OS 分布/CPU 核数/内存容量多维度可视化，扫描成功率摘要（交换机/vCenter/F5/ZDNS/IP扫描/椒图/鼎甲备份/资产同步/信息系统同步9类数据源中文标签），可用 IP 查询
- **交换机管理** — CRUD、批量导入(Excel/CSV)、模板下载、SNMP 连接测试、扫描状态/结果展示、全部扫描/全部删除
- **SNMP 扫描** — 自动识别 L2/L3 交换机，支持标准 MIB 和华为私有 MIB（含 BD 模式），华为 SDN 多路径 OID 回退 + ipForwardTable 路由采集，ARP+FDB 多源合并，空 IP 自动回填
...
- **智能搜索** — 顶栏全局搜索按输入类型智能路由：域名→资产画像、IP/MAC→IP地址信息、其他→信息系统维护，各模块页面内搜索
...
- **ZDNS 域名管理** — CRUD、连接测试、REST API 分页采集 DNS 记录，域名→IP/别名映射（A/AAAA/CNAME，自动过滤反向解析记录），定时扫描，历史追踪
- **ZDNS 记录与映射** — DNS 记录和域名→IP/别名映射清单各自搜索分页，支持记录类型(A/AAAA/CNAME)/启用状态/IP 状态筛选
- **ZDNS IP 可达性扫描** — 关联交换机和 F5 数据判定 IP 在线状态（在线/离线/禁用/待定），一键 ping 探测 + 定时 IP 扫描，独立扫描间隔
- **信息资产管理** — 部门树+标签页（虚拟机/域名/信息系统），VM 自动分组（按 vm_folder 匹配部门）、匹配负责人（从备注提取人名→API查询教职工→部门链匹配，仅处理未认领 VM 避免重复设置）、批量认领/指派/撤销、分组预览确认
...
- **鼎甲备份对接** — 备份记录自动采集，/d2/r/v2/sets API 获取每个VM的备份集（minor_resource_name/backup_start_time/backup_end_time），时区修正（UTC→东八区），持续时间计算，可展开历史版本子表（按时间倒序），默认开始时间倒序+VM名称正序，分页排序/记录统计
...
| GET | /api/sys/departments/tree | 组织树（所有用户，支持全部部门） |
| GET | /api/sys/departments/{id}/users | 部门下用户列表（管理员） |
| GET | /api/info-systems/staff-search | 人员搜索（所有用户，系统用户搜索） |
| GET | /api/info-systems/staff-lookup | 人员查找（所有用户，系统用户+教职工库+数据中台三源检索） |
| POST | /api/info-systems/staff-register | 人员注册（所有用户，自动查询教职工并注册为系统用户） |
| GET | /api/assets/departments/{id}/vms | 部门 VM 清单（分页/筛选/增强数据） |
| GET | /api/assets/departments/{id}/domains | 部门域名清单（去重/分页/类型/认领/填报状态筛选） |
...
| GET | /api/zdns/{id}/domain-map | 域名→IP/别名映射清单（A/AAAA/CNAME，过滤反向解析） |
...
| CRUD | /api/info-systems | 信息系统维护（分页/搜索/筛选） |
| POST | /api/info-systems/import | 信息系统 Excel 导入 |
| GET | /api/info-systems/export | 信息系统 Excel 导出 |
| POST | /api/info-systems/sync-from-platform | 域名同步（手动触发，写入任务日志） |
| CRUD | /api/info-systems/djdj | 等保备案记录维护 |
| POST | /api/info-systems/djdj/import | 等保 Excel 导入 |
| GET | /api/info-systems/djdj/export | 等保 Excel 导出 |
| POST | /api/info-systems/djdj/batch-delete | 等保批量删除 |
| CRUD | /api/info-systems/icp | ICP 备案记录维护 |
| POST | /api/info-systems/icp/import | ICP Excel 导入 |
| GET | /api/info-systems/icp/export | ICP Excel 导出 |
| CRUD | /api/info-systems/supply-chain | 供应链信息维护（分页/搜索） |
| POST | /api/info-systems/supply-chain/import | 供应链 Excel 导入（自动去重+行业名称适配） |
| GET | /api/info-systems/supply-chain/export | 供应链 Excel 导出 |
| POST | /api/info-systems/supply-chain/batch-delete | 供应链批量删除 |
| GET | /api/info-systems/supply-chain/names | 供应链单位名称列表（下拉联动） |
...
| Phase M | 资产数据独立 — asset_inventory 表隔离管理数据与扫描数据 | ✅ |
| Phase N | 鼎甲备份系统对接 — REST API + Celery 任务队列 + 扫描日志 | ✅ |
| Phase O | 信息系统管理 — 信息系统/等保/ICP/供应链四大模块，Excel 导入导出，批量删除 | ✅ |
...
| 交换机管理 | SNMP v2c 采集（标准 MIB + 华为私有 MIB），L2/L3 自动识别，ARP+FDB 合并，路由表（ipRouteTable + ipForwardTable），SDN 多路径回退 | ✅ |
...
| 地址段管理 | CRUD + IP 可用性计算 + 利用率统计 + 已占用 IP 明细 + 订阅他人地址段 + 逐条仪表盘展示控制 | ✅ |
...
| F5 系统运维 | 虚拟服务器视图（irule域名+ZDNS存活+内网服务器Pool/iRule来源标签+成员去重）、成员池视图（运行状态+引用状态筛选+VS/iRules引用含iRule溯源+空成员池补全）、iRules视图（域名→Pool映射+端口后缀清洗+switch/if多场景正则识别），客户端全量分页，管理员专属 | ✅ |
| 系统监控 | 资产关联（域名↔F5 VS↔成员IP↔VM↔备份/椒图全链路自动绑定+QAX IP精确匹配+备注自动带入+角色智能提示），系统拓扑图（ECharts分层无漂移+内圈类别色+外圈4px状态圈+中文图例+数据库位置持久化+成员下挂VM+完整VS/成员节点），统计数字点击筛选，三级权限过滤 | ✅ |
| 交互优化 | 全系统多选表格固定首列（复选框+名称）横滚不动 + 点击行任意位置切换选中，覆盖8处表格 | ✅ |
| 时区统一 | MySQL `default-time-zone=+08:00` + 后端 `app/utils/timezone.py` 共享 `now()` + 前端 `utils/format.js` 共享 `formatTime`/`formatDate`（14 文件统一导入，删除重复定义），全系统时间显示东八区 | ✅ |
...
| 信息系统管理 | 信息系统台账/等保备案/ICP备案/供应链信息维护，Excel 导入导出，域名自动同步 | ✅ |
