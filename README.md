...
| 信息资产管理 | VM/域名/信息系统关联部门，自动匹配(vm_folder→部门编码)，管理员指派（人员搜索+部门同步），资产同步（新增标记unlinked+僵尸清理），文件夹树（/分隔路径，隐藏空节点，随部门联动），默认排序（资源池→名称→电源），外链填报（加密token+可选密码+过期时间+独立认证+外部填报保存），普通用户权限开放（部门树/人员搜索/人员注册），删除失败明确提示原因 | ✅ |
