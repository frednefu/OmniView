...
- **资产画像** — 跨 ZDNS/F5/vCenter/交换机 四源数据关联，自动构建 域名→公网IP→端口→内网服务IP:端口→虚拟机→IP/MAC/网络/VLAN/文件夹 完整链路；融合来源字段（多彩标签展示 ZDNS/F5/vCenter/Switch 组合），多维度筛选（来源精确匹配/状态/网络名称下拉），统计卡片（域名/公网IP+端口/虚拟机/VM IP+端口/VLAN/文件夹数量），全局搜索，多列排序，分页
