...
- **信息系统管理** — 信息系统资产台账维护（36字段：基本情况/归属信息/等级保护/供应链/ICP备案），Excel 三格式智能导入（自动识别资产导出.xlsx/资产清单.xlsx/信息系统管理_*.xlsx + 表头驱动动态映射 + 3种去重策略：补充空白/覆盖已有/跳过重复），TRIM 系统名称匹配防重复新建，Excel 36列中文全字段导出，批量删除/字段排序，数据同步（域名比对 ZDNS 离线 IP 自动回填），人员查询两步确认 + 自动注册（fetch_staff 外部 API 补全邮箱/电话/部门/性别），等保系统名称本地下拉联动等保信息维护表，测评单位独立字段，开发厂商联动供应链信息维护表，导入时供应链厂商自动入库 + 分组翻页统一，ICP 备案信息维护，供应链信息维护（CRUD/批量删除/Excel导入导出/数据统计），asset_id 数据库查重防碰撞
