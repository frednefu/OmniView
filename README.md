...
- **信息系统管理** — 信息系统资产台账维护（36字段：基本情况/归属信息/等级保护/供应链/ICP备案），Excel 多格式智能导入（自动识别资产导出/信息系统管理两种格式 + 3种去重策略：补充空白/覆盖已有/跳过重复），Excel 全字段导出，批量删除，数据同步（域名比对 ZDNS 离线 IP 自动回填），人员查询两步确认 + 自动注册，等保系统名称联动等保信息维护表，开发厂商联动供应链信息维护表，等保信息维护（备案证明图片上传/Excel导入导出），ICP 备案信息维护，供应链信息维护（CRUD/批量删除/Excel导入导出/数据统计）
